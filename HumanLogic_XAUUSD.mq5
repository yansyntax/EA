//+------------------------------------------------------------------+
//|                                             HumanLogic_XAUUSD.mq5|
//|                                Copyright 2026, Expert Advisor    |
//+------------------------------------------------------------------+
#property copyright "Expert Advisor XAUUSD"
...
input double   InpTrailingPips  = 2.5;       // Jarak SL+ menguntit di belakang harga running (Pips)
...
//--- Locked Parameters for XAUUSD
const double FIXED_LOT_SIZE = 0.05;          // Fixed Lot 0.05 untuk XAUUSD
...
void OnTick()
  {
   ...
   if(isH1Bullish && rates[0].close > rates[0].open && rates[1].close < rates[1].open)
     {
      ...
      trade.Buy(FIXED_LOT_SIZE, _Symbol, ask, sl, tp, "HumanLogic XAUUSD Buy");
     }
   ...
   // Logika Sell
   else if(isH1Bearish && rates[0].close < rates[0].open && rates[1].close > rates[1].open)
     {
      ...
      trade.Sell(FIXED_LOT_SIZE, _Symbol, bid, sl, tp, "HumanLogic XAUUSD Sell");
     }
  }
      ...
      if(ticket > 0 && PositionGetString(POSITION_SYMBOL) == _Symbol)
        {
         ...
         if(type == POSITION_TYPE_BUY)
           {
            // A. Emergency Cut jika berjarak mendekati TP tapi mulai berbalik
         ...
         // --- POSISI SELL ---
         else if(type == POSITION_TYPE_SELL)
           {
            ...
            // C. Trailing SL+ (Ikut bergeser TURUN saat harga makin turun)
