//+------------------------------------------------------------------+
//|                                              HumanLogic_Vol80.mq5|
//|                                Copyright 2026, Expert Advisor    |
//+------------------------------------------------------------------+
#property copyright "Expert Advisor Vol80"
...
//--- Locked Parameters for Volatility 80
const double FIXED_LOT_SIZE = 0.01;          // Fixed Lot 0.01 untuk VOL 80
...
void OnTick()
  {
   ...
   if(isH1Bullish && rates[0].close > rates[0].open && rates[1].close < rates[1].open)
     {
      ...
      trade.Buy(FIXED_LOT_SIZE, _Symbol, ask, sl, tp, "HumanLogic Vol80 Buy");
     }
   ...
   // Logika Sell
   else if(isH1Bearish && rates[0].close < rates[0].open && rates[1].close > rates[1].open)
     {
      ...
      trade.Sell(FIXED_LOT_SIZE, _Symbol, bid, sl, tp, "HumanLogic Vol80 Sell");
     }
  }
